...
public class BertTokenizer {
    ...
    public func vectorize(s1: String, s2: String, max_sentence_len: Int = 253) -> ([Int],[Int],[Int]) {
        var tokens_a = self.tokenize(text: s1)
        var tokens_b = self.tokenize(text: s2)
        if tokens_a.count > max_sentence_len {
//            print(tokens_a)
            tokens_a = Array(tokens_a[..<max_sentence_len])
//            print(tokens_a)
        }
        
        if tokens_b.count > max_sentence_len {
            tokens_b = Array(tokens_b[..<max_sentence_len])
        }
        ...
        let input_ids = try! self.convertTokensToIds(tokens: tokens)
        let input_mask = [Int](repeating: 1, count: input_ids.count )
