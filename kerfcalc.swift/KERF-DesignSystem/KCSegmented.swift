...
/// Pill segmented control for a tool's sub-screens — drop-in for the existing
/// `SubScreenPicker(titles:selection:)` API. In the light theme the selected pill
/// fills with graphite (the same "active" language as the tab bar and keypad),
/// with a 2px section-accent underline. Set `.tint(tool.accent)` at screen level.
