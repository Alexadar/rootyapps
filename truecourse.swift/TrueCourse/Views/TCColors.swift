...
struct TCPalette {
    ...
    // The single source of wayfinding colour: one accent per catalog group (Planning,
    // Performance, Tools), via `accent(_:)` below. In `night`, accents collapse to a warm
    // monochrome-red scale so no cool light reaches the flight deck — the grouping survives,
    // the blue does not.
    ...
    static let dark = TCPalette(
        ...
        accentMap: [
            .planning:    Color(rgbHex: 0x35C4DE),   // teal
            .performance: Color(rgbHex: 0x47D18A),   // green
            .tools:       Color(rgbHex: 0x8B98A9),   // grey
        ])
    ...
    static let night = TCPalette(
        ...
        accentMap: [
            .planning:    Color(rgbHex: 0xFF7B5E),
            .performance: Color(rgbHex: 0xFF8A66),
            .tools:       Color(rgbHex: 0xC85A44),
        ])

    // Storage for the accent map; `accent(_:)` reads it.
    private let accentMap: [CalcGroup: Color]
...
extension TCPalette {
    func accent(_ g: CalcGroup) -> Color { accentMap[g] ?? brand }
    /// Convenience: a tool's accent is its group's accent (one accent per group).
    func accent(_ tool: Tool) -> Color { accent(tool.group) }
}
...
// MARK: - Catalog groups + accents

/// The three catalog families. Each owns one accent — the wayfinding system.
/// The raw value is the section header shown in the catalog.
enum CalcGroup: String, CaseIterable, Identifiable, Hashable {
    case planning    = "Planning & Navigation"   // wind · nav · fuel · climb
    case performance = "Performance"             // airspeed · altitude · weight & balance
    case tools       = "Tools"                   // convert · timer
    var id: String { rawValue }
}
