...
def sapo_step(env, dparams, dls, eparams, els, dopt, log_alpha, aopt, P, H_dec, K_dec=80, dtarget=None,
    ...
    PPO bootstrap). It is SHAC's windowed analytic-gradient BPTT (see shac_step) made MAXIMUM-ENTROPY:
    ...
    Everything else matches shac_step: reparameterised action, EMA target net for the value bootstrap,
    ...
    while k < K_dec:                                         # SAPO-EPISODE-LOOP-OK: walk the episode window-by-window
        ...
        with torch.no_grad():                                # targets are constants for the regression -> detach everything
            ...
            # SOFT TD(lambda) return-to-go, VECTORIZED (no scan). Same triangular-matrix unroll as shac_step, but the
