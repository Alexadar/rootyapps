...
public struct HpoPanel: Equatable, Codable {
    ...
    /// Highest Hp30 of the 24 h ending at the newest reading.
    ///
    /// Mirrors `KpPanel.peak24h(asOf:)` deliberately: Hp30 is now the headline geomagnetic value
    /// everywhere, so its peak has to be the one shown beside it — pairing an Hp30 "now" with a Kp
    /// 24-hour peak would put two different indices in one readout and invite the comparison to be
    /// read as a trend. Both ends of the window are bounded; GFZ publishes no future readings, but
    /// the guard costs nothing and the Kp equivalent needed it.
    public func peak24h(asOf date: Date = Date()) -> Double? {
        guard let end = readings.last(where: { $0.time <= date })?.time else { return nil }
        let cutoff = end.addingTimeInterval(-24 * 3600)
        return readings.filter { $0.time >= cutoff && $0.time <= end }.map(\.value).max()
    }
...
public struct SpaceWeatherSnapshot: Equatable, Codable {
    ...
    // MARK: - Current geomagnetic level

    // One quantity, two publishers of differing precision. NOAA's planetary Kp is a 3-hourly
    // Bartels interval published 3–5 h in arrears; GFZ's Hp30 is the SAME 0–9 geomagnetic scale
    // sampled every 30 minutes and published ~40 min behind. So Kp cannot show a storm that is
    // starting: measured 2026-08-02, Kp sat on 1.0 from the 09:00 interval while Hp30 ran
    // 1.33 → 1.67 → 3.33 → 4.67 over the same 90 minutes.
    //
    // Hp30 is therefore the oracle for the geomagnetic readout, with Kp as the fallback when GFZ
    // has published nothing. These accessors exist so that decision lives in ONE place: the views
    // ask "what is the level now", not "which feed should I prefer today". Every readout must
    // agree, and a per-view `hpo ?? kp` would drift the first time one was edited alone.

    /// The current geomagnetic level, from the most precise source available.
    public var geomagNow: Double? { hpo?.latest ?? kp?.now }

    /// Highest level of the last 24 h, from the same source as `geomagNow` — never mixed, or the
    /// pair reads as a trend between two different indices.
    public func geomagPeak24h(asOf date: Date = Date()) -> Double? {
        hpo != nil ? hpo?.peak24h(asOf: date) : kp?.peak24h(asOf: date)
    }

    /// NOAA G-scale for the current level. Hpo is defined on the Kp scale, so the same
    /// oracle-tested classifier applies to either source.
    public var geomagGScale: Int { geomagNow.map { Geomag.gScale(forKp: $0) } ?? 0 }

    /// Which publisher `geomagNow` came from — the panel must cite the source it actually used.
    public var geomagSource: String { hpo != nil ? "GFZ Potsdam · Hp30" : "NOAA SWPC · Kp" }

    /// When that source measured it, for the age badge.
    public var geomagObservedAt: Date? { hpo != nil ? hpo?.observedAt : kp?.observedAt }
}
