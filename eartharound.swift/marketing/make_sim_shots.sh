...
# EA-, not Calc-: these defaults were a copy-paste from kerfcalc and named ANOTHER app's
# devices, so a sweep either aborted (once they were deleted) or trampled a concurrent session
# capturing kerfcalc in the same simulator. ensure_sim machines the device up on demand.
case "$PLATFORM" in
  ipad) SIM_NAME="${SIM_NAME:-EA-iPadPro13}";      SIM_TYPE="iPad Pro 13-inch" ;;
  *)    SIM_NAME="${SIM_NAME:-EA-iPhone17ProMax}"; SIM_TYPE="iPhone 17 Pro Max" ;;
esac
...
. "$ROOT/marketing/simlib.sh"
UDID=$(ensure_sim "$SIM_NAME" "$SIM_TYPE" iOS)
[ -n "$UDID" ] || { echo "❌ could not create simulator '$SIM_NAME'"; exit 1; }
