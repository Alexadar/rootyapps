...
struct WatchReadoutPage: View {
    ...
    // Preciser oracle for the same readout — see `SpaceWeatherSnapshot.geomagNow`.
    private var kpNow: Double { snapshot.geomagNow ?? 0 }
    private var gScale: Int { snapshot.geomagGScale }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(snapshot.geomagNow != nil ? Fmt.num(kpNow, 1) : "—")
            ...
            HStack(spacing: 12) {
                // Highest level that actually occurred in the window — forecast rows excluded.
                watchStat(label: SWText.str("24H PEAK"),
                          value: Fmt.num(snapshot.geomagPeak24h(), 1))
