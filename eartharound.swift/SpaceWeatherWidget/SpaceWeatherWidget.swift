...
struct SpaceWeatherProvider: TimelineProvider {
    ...
    static func fetchEntry() async -> SpaceWeatherEntry {
        ...
        if let reason = NetworkMonitor.shared.blockedReason(cellularAllowed: cellularAllowed) {
            ...
            let now = Date()
            for source in [FeedSource.kp, .hpo, .scales, .wind, .aurora, .flares] { status.record(source, reason, at: now) }
            shared.status = status
            return SpaceWeatherEntry(date: now, snapshot: snapshot, refreshedAt: cached?.at)
        }

        // Hp30 is fetched HERE, not inherited from the app's cache. The widget's headline is
        // `geomagNow`, which prefers Hp30 — so a widget that refreshed Kp but not Hp30 rendered a
        // freshly-fetched timestamp beside a level the app had cached hours earlier. Six sources
        // now, not five.
        ...
        accept(.hpo, hpoP) { fresh.hpo = $0; snapshot.hpo = $0 }
        ...
        accept(.aurora, auroraP) { a in
            let panel = AuroraPanel(maxProbability: a.max, kp: snapshot.geomagNow ?? 0, observedAt: a.at)
...
struct SpaceWeatherWidgetView: View {
    ...
    // The geomagnetic level, from the preciser of the two publishers — see
    // `SpaceWeatherSnapshot.geomagNow`. The G badge is derived from that same value rather than
    // from NOAA's published scale, which is computed off the 3-hourly Kp: taking one from each
    // would let the widget show an Hp30 level next to a G-scale describing an earlier interval.
    private var kpNow: Double { entry.snapshot.geomagNow ?? 0 }
    private var gScale: Int { entry.snapshot.geomagGScale }
    private var auroraPct: Int? { entry.snapshot.aurora?.maxProbability }
    private var windKms: Double? { entry.snapshot.wind?.speed }
    private var hasData: Bool { entry.snapshot.geomagNow != nil }
    ...
    private var medium: some View {
        ...
        HStack(spacing: 14) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                // geomagObservedAt, NOT kp.observedAt: this column's number comes from
                // `geomagNow`, so its age has to come from the same source. Pairing an Hp30
                // value with the Kp panel's timestamp labelled a 40-minute-old reading
                // "measured 5 hr ago".
                columnFooter(entry.snapshot.geomagObservedAt)
            }
    ...
    /// When the SOURCE measured this column's data — the freshness of the reading, which is the
    /// question worth answering. Our own poll time is not shown: it says nothing about whether the
    /// number is current.
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 12) {
                ...
                stat(label: SWText.str("24H PEAK"),
                     value: Fmt.num(entry.snapshot.geomagPeak24h(), 1))
                stat(label: "WIND", value: windKms.map { Fmt.num($0, 0) } ?? "—", unit: "KM/S")
            }
        }
    }
