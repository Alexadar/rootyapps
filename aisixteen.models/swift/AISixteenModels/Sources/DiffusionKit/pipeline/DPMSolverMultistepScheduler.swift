...
public final class DPMSolverMultistepScheduler: Scheduler {
    ...
    /// Widened from `private(set)` so a resumed run can put it back. See `Resumption.swift`.
    public internal(set) var lowerOrderStepped = 0
    ...
    /// Widened from `private(set)` for the same reason as `lowerOrderStepped`.
    public internal(set) var modelOutputs: [MLShapedArray<Float32>] = []
