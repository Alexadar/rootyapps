...
public struct PipelineConfiguration: Hashable {
    ...
    /// Where to pick a de-noising loop up from, if it is being resumed. `nil` for a fresh run.
    /// Honoured only for `imageCount == 1`; a checkpoint carries one image's state.
    public var resumePoint: ResumePoint? = nil
