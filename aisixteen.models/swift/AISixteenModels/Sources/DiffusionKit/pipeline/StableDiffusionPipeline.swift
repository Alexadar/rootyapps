...
public struct StableDiffusionPipeline: StableDiffusionPipelineProtocol {
    ...
    /// Keep the unet and the ControlNet resident when `generateImages` returns.
    ///
    /// `reduceMemory` unloads **every** model at the end of every call, which is right for a one-shot
    /// generation and badly wrong for a caller that is about to call again immediately. A tiled
    /// refinement calls `generateImages` once per tile — nine times for a 1024² wallpaper — so the
    /// 618 MB unet and the ControlNet are torn down and read back from disk eight times for nothing.
    ///
    /// Measured over a nine-tile refinement on this Mac, together with the autorelease pools added
    /// to the de-noising loop and the tile loop in the same change:
    ///
    /// | | peak | wall time |
    /// |---|---|---|
    /// | before: unload every tile, no pools | 1.43 GB | 372 s |
    /// | before: no `reduceMemory`, no pools | 2.55 GB | 40 s |
    /// | **after: retain these two, with pools** | **0.46 GB** | **42 s** |
    ///
    /// Nine times faster on a third of the memory. Retention buys the time — these two are resident
    /// *during* a tile either way, so keeping them across the gap between tiles costs almost nothing
    /// and saves eight reloads of 868 MB. The pools buy the memory, and they buy most of it: without
    /// one, every array Core ML autoreleased over nine tiles lived until the whole refinement
    /// returned. That is what made an Enhance die at the eighth tile of nine, and what put the
    /// device under enough sustained pressure to respring and reboot.
    ///
    /// The text encoder, the VAE encoder and the decoder keep unloading, because each is used once
    /// per tile and is a fraction of the size.
    ///
    /// The caller is then responsible for `unloadResources()` when the loop ends.
    public var retainsDenoisingModels: Bool = false
    ...
    public func generateImages(
        ...
        // Resuming: throw away the freshly sampled noise and put the interrupted run's state back.
        // The seed still matters — it produced the latents being restored — but it is no longer what
        // the loop starts from.
        //
        // Restricted to a single image because a checkpoint carries one image's latents and one
        // scheduler's outputs. The app only ever asks for one; more would need a checkpoint per
        // image, and silently resuming the wrong one is worse than not resuming.
        let resumePoint: ResumePoint? = config.imageCount == 1 ? config.resumePoint : nil
        if let resumePoint, resumePoint.latents.count == latents.count {
            latents = resumePoint.latents
            // What the decoder eventually sees. `step` sets it from `modelOutputs.last`, so a resumed
            // run that stopped before producing any would still be holding the initial latents.
            denoisedLatents = [resumePoint.modelOutputs.last ?? latents[0]]
            for scheduler in scheduler {
                (scheduler as? DPMSolverMultistepScheduler)?
                    .restore(modelOutputs: resumePoint.modelOutputs,
                             lowerOrderStepped: resumePoint.lowerOrderStepped)
            }
        }
        ...
        // Zero residuals for a controlled unet running without a ControlNet. `nil` for a plain
        // unet, in which case nothing changes.
        let zeroResiduals: [[String: MLShapedArray<Float32>]]? =
            controlNet == nil ? try unet.zeroResidualsIfNeeded() : nil

        // De-noising loop
        var stopped = false
        let timeSteps: [Int] = scheduler[0].calculateTimesteps(strength: timestepStrength)
        let alreadyDone = resumePoint?.step ?? 0
        for (step,t) in timeSteps.enumerated() {
          // Core ML autoreleases the arrays and buffers each prediction produces. Without a pool
          // per step they accumulate until this whole function returns — which for a tiled
          // refinement is minutes and hundreds of allocations, on a device that reboots when the
          // system runs short.
          try autoreleasepool {

            // The whole of stage 1 resumption: the steps before the checkpoint are skipped, and the
            // state they produced was restored above. Skipping rather than slicing `timeSteps` keeps
            // `step` and `stepCount` meaning the same thing to the progress handler, so a resumed
            // run reports "step 18 of 28" and not "step 4 of 14".
            if step < alreadyDone { return }
            ...
            //
            // When there is no ControlNet but the unet was converted with `--unet-support-controlnet`,
            // its residual inputs are still required and must be filled — with zeros, which is
            // bit-identical to a plain unet. That is what lets this app ship ONE unet for both
            // text-to-image and tile refine instead of two. `zeroResiduals` is built once outside
            // the loop; allocating thirteen tensors on every step would be pure waste.
            ...
            let progress = Progress(
                ...
                // `step + 1` — steps *completed*, which is exactly what a resumed loop skips. Off by
                // one here and a resumed run silently repeats or skips a step, which changes the
                // picture without failing anything.
                resumeState: (scheduler[0] as? DPMSolverMultistepScheduler)?
                    .resumeState(completedSteps: step + 1, latents: latents),
        ...
        if reduceMemory && !retainsDenoisingModels {
            controlNet?.unloadResources()
            unet.unloadResources()
        }
...
public struct PipelineProgress {
    ...
    /// Everything needed to write a checkpoint, as of the step that just finished. A `var` with a
    /// default so the two pipelines that do not support resumption construct `Progress` unchanged.
    public var resumeState: ResumeState? = nil
