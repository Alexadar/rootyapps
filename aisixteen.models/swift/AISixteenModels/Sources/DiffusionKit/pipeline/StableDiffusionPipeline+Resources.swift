...
public extension StableDiffusionPipeline {
    ...
    init(
        ...
        if controlNet == nil {
            // A bundle converted with `--unet-support-controlnet` contains ONLY `ControlledUnet`.
            // Fall back to it so text-to-image works from a single-unet pack; the residual inputs
            // are then filled with zeros, which is bit-identical to a plain unet.
            let plainExists = FileManager.default.fileExists(atPath: urls.unetURL.path)
                || FileManager.default.fileExists(atPath: urls.unetChunk1URL.path)
            unetURL = plainExists ? urls.unetURL : urls.controlledUnetURL
            unetChunk1URL = plainExists ? urls.unetChunk1URL : urls.controlledUnetChunk1URL
            unetChunk2URL = plainExists ? urls.unetChunk2URL : urls.controlledUnetChunk2URL
        } else {
