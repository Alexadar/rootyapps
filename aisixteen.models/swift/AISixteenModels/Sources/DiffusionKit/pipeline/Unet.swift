...
public struct Unet: ResourceManaging {
    ...
    /// Widened from `internal` to `public` — the ONLY change to Apple's source in this file.
    ///
    /// The app ships a single `ControlledUnet` and drives it both ways: real residuals for tile
    /// refine, and thirteen zero tensors for plain text-to-image, which is bit-identical to a plain
    /// unet. Apple's own `StableDiffusionPipeline` will only reach this with a ControlNet attached,
    /// so text-to-image on a controlled unet is unreachable from outside the module.
