...
///
/// Oracle class: IDENTITY, cross-checked against the numbers printed on a physical framing square's
/// rafter table. common = √(rise² + 12²) per foot of run; hip/valley = √(rise² + 288) per foot of
/// *common* run, because a regular hip's plan run is the 45° diagonal (unit run 12√2 = 16.97″).
///
/// Reference: NAVEDTRA 14044 "Framing Roofs" (US Navy, public domain); framing-square rafter table.
/// Ported from `kerfcalc.swift/Kits/Framing/FramingKit/Sources/FramingKit/Rafter.swift`.
public enum Rafter {
    ...
    /// Plumb-cut angle (from level), degrees — the tail/ridge cut of a common rafter.
    ...
    /// Slope factor — rafter length per inch of horizontal run.
    ...
    public static func jackLength(rise: Double, longestRunFeet: Double,
                                  spacingInches: Double, index n: Int) -> Double {
    ...
    /// Ridge shortening, measured ALONG the rafter: half the ridge thickness × slope factor.
    ...
    /// Actual rafter cut length, inches — ridge-plumb to tail-plumb. This is what a framer cuts.
    public static func actualLength(rise: Double, runFeet: Double,
                                    ridgeThicknessIn: Double, overhangIn: Double) -> Double {
