...
public final class BatchWorld {
    ...
    /// Advance all N envs by one tick on the GPU.
    public func step() {
        t += 1
        let elapsed = MLXArray(Float(t) * dt)

        // --- activate due spawns (masked write) ---
        let due = spawnTick .<= elapsed                               // [N,M] bool
        let justAct = MLX.logicalAnd(due, monActivated .< 0.5)
        let actPos = playerPos.expandedDimensions(axis: 1) + offset   // [N,M,2]
        monPos = MLX.where(justAct.expandedDimensions(axis: 2), actPos, monPos)
        monActivated = MLX.maximum(monActivated, justAct.asType(.float32))
        let aliveBefore = MLX.logicalAnd(due, monHP .> 0)             // [N,M] bool

        // --- steering toward player ---
        let rel = playerPos.expandedDimensions(axis: 1) - monPos      // [N,M,2] toward player
        let dist = MLX.sqrt((rel * rel).sum(axis: 2)) + eps           // [N,M]
        let dir = rel / dist.expandedDimensions(axis: 2)              // [N,M,2]
        let stop = playerRadius + monBoxW / 2                          // [N,M]
        let moveMask = MLX.logicalAnd(aliveBefore, dist .> stop)      // [N,M]
        ...
        // --- auto-fire toward monster centroid (scatter-free, fixed cadence) ---
        if t % fireInterval == 0 {
            let toMon = monPos - playerPos.expandedDimensions(axis: 1)        // [N,M,2]
            let aliveF = aliveBefore.asType(.float32).expandedDimensions(axis: 2)
            let sumDir = (toMon * aliveF).sum(axis: 1)                        // [N,2]
            let aim = sumDir / (MLX.sqrt((sumDir * sumDir).sum(axis: 1, keepDims: true)) + eps)
            let slot = (t / fireInterval) % b
            var oneHot = [Float](repeating: 0, count: b); oneHot[slot] = 1
            let sMask = MLXArray(oneHot, [1, b, 1])                           // [1,B,1]
            let freshPos = playerPos.expandedDimensions(axis: 1)             // [N,1,2]
            let freshVel = (aim * bulletSpeed).expandedDimensions(axis: 1)   // [N,1,2]
            bulPos = MLX.where(sMask .> 0.5, freshPos, bulPos)
            bulVel = MLX.where(sMask .> 0.5, freshVel, bulVel)
            let sMask2 = MLXArray(oneHot, [1, b])
            bulAlive = MLX.where(sMask2 .> 0.5, MLXArray(Float(1)), bulAlive)
            bulDist = MLX.where(sMask2 .> 0.5, MLXArray(Float(0)), bulDist)
        }

        // --- bullets move + expire by range ---
        ...
        // --- collisions bullet x monster (reduction, no scatter) ---
        let bpos = bulPos.expandedDimensions(axis: 2)                 // [N,B,1,2]
        let mpos = monPos.expandedDimensions(axis: 1)                 // [N,1,M,2]
        let diff = bpos - mpos
        let d2 = (diff * diff).sum(axis: 3)                           // [N,B,M]
        let hitRange = (bulletRadius + monBoxW / 2)                   // [N,M]
        let hitR2 = (hitRange * hitRange).expandedDimensions(axis: 1)// [N,1,M]
        let bulA = (bulAlive .> 0.5).expandedDimensions(axis: 2)      // [N,B,1]
        let monA = aliveBefore.expandedDimensions(axis: 1)           // [N,1,M]
        let hit = MLX.logicalAnd(d2 .< hitR2, MLX.logicalAnd(bulA, monA)).asType(.float32)  // [N,B,M]
        let dmgToMon = (hit * bulletDamage).sum(axis: 1)             // [N,M]
        monHP = monHP - dmgToMon
        let bulHits = hit.sum(axis: 2)                               // [N,B]
        bulAlive = MLX.logicalAnd(bulAlive .> 0.5, bulHits .< 0.5).asType(.float32)  // penetration 1

        // --- kills this step ---
        let aliveAfter = MLX.logicalAnd(due, monHP .> 0)
        let killed = MLX.logicalAnd(aliveBefore, aliveAfter .< 0.5).asType(.float32).sum(axis: 1)
        killCount = killCount + killed

        // --- periodic contact damage ---
        if t % contactInterval == 0 {
            let contactRange = playerRadius + monBoxW / 2 + buffer    // [N,M]
            let contact = MLX.logicalAnd(aliveAfter, dist .< contactRange).asType(.float32)
            let dmg = (contact * monDamage).sum(axis: 1)              // [N]
            playerHP = playerHP - MLX.maximum(dmg - playerDefense, MLXArray(Float(0)))
        }

        // --- scripted player kite: move away from monster centroid ---
        let toMon2 = monPos - playerPos.expandedDimensions(axis: 1)
        let aliveF2 = aliveAfter.asType(.float32).expandedDimensions(axis: 2)
        let centroidDir = (toMon2 * aliveF2).sum(axis: 1)            // [N,2]
        let away = centroidDir * MLXArray(Float(-1))
        let awayN = away / (MLX.sqrt((away * away).sum(axis: 1, keepDims: true)) + eps)
        let np = playerPos + awayN * (playerSpeed * dt)
        let lo = -mapHalf + playerHalf, hi = mapHalf - playerHalf
        playerPos = MLX.clip(np, min: MLXArray(lo), max: MLXArray(hi))

        eval(monPos, monVel, monHP, monActivated, bulPos, bulVel, bulAlive, bulDist, playerPos, playerHP, killCount)
    }
