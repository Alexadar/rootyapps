...
final class Game {
    ...
    init(mapPath: String = "../torchsim/datasets/tiny/eval/e3.json",
         ...
         enemyPath: String = "models/monster.mlmodel", playerPath: String = "",
        ...
        typeRot = rotOffsets
        guard let enemy = CoreMLNet(path: enemyPath) else {
            throw NSError(domain: "game", code: 2, userInfo: [NSLocalizedDescriptionKey: "no enemy model at \(enemyPath)"])
        }
        let modelPlayer = playerPath.isEmpty ? nil : CoreMLNet(path: playerPath)
