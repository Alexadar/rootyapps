...
"$PY" train_torch.py --dataset datasets/surround --perm 16 --pop 256 --cap 32 --bullets "${BULLETS:-24}" --ticks 600 \
  ...
  --iters 100000 --budget "${BUDGET:-600}" --eval-every "${EVAL_EVERY:-15}" --eval-vs scripted --eval --eval-panel \
  --logdir "$LOG" --weapons "${WEAPONS:-1,5,4}" \
  ...
  --rw-align 0.01 --rw-separate 0.02 --sep-radius 50 \
  --rw-e-approach "$RW_E_APPROACH" \
