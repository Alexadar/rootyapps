...
struct ContentView: View {
    ...
    var body: some View {
        NavigationSplitView {
            ...
            if let selection {
                // `ToolScreen` already renders the tool name as the in-content title,
                // under the "MARINE NAV" eyebrow. Setting it here too printed it twice
                // on iOS (inline bar + content). The bar keeps its back button either
                // way, which is what ReelTour taps.
    ...
    private var sidebar: some View {
        List(Tool.allCases, selection: $selection) { tool in
            ...
            // On the LINK, not on the HStack inside it: ReelTour's primary query is
            // `app.buttons["tool.<raw>"]`, and an identifier on an inner view leaves
            // the row itself unlabelled, forcing the tour onto its silent fallbacks.
