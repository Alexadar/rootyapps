...
struct WatchDeclinationView: View {
    ...
    /// VoiceOver text for the variation readout. Kept out of the view body — assembled
    /// inline it exceeded the type-checker's budget. Speaks the direction as a WORD, per
    /// the accessibility rule that a label never relies on a glyph or a colour.
    static func variationLabel(_ value: String, _ declinationDeg: Double) -> String {
        let side = declinationDeg >= 0 ? "east" : "west"
        return "Variation \(value) degrees \(side), magnetic north lies \(side) of true north."
    }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if let (coord, live) = coordinate, let f = model.field(at: coord) {
                        HStack(alignment: .center, spacing: 8) {
                            ...
                            // Built by a plain function, not inline: the four-way string
                            // concatenation with two embedded ternaries and an interpolation
                            // blew the type-checker's budget here ("unable to type-check this
                            // expression in reasonable time").
                            .accessibilityLabel(Self.variationLabel(model.value(f.declinationDeg),
                                                                    f.declinationDeg))
                            Spacer(minLength: 2)
                        ...
                        .digitalCrownRotation($model.trueHeading,
                                              ...
                                              // spinning; .high keeps 1° precision, less travel.
                                              sensitivity: .high,
                                              isContinuous: true,
