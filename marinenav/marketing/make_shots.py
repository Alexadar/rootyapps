#!/usr/bin/env python3
"""Compose Marine Nav App-Store screenshots (6.9" iPhone, 1320x2868) from raw sim
...
Run: marketing/venv/bin/python3 marketing/make_shots.py
"""
...
def compose(name, headline, sub):
    ...
    # wordmark
    fw = font(BOLD, 44)
    d.text((92, 104), "MARINE NAV", font=fw, fill=AQUA)

    # headline
    fh = wrap_lines(d, headline, font(BOLD, 104), W - 184)
    y = 196
    for ln in headline:
        d.text((92, y), ln, font=fh, fill=INK)
        y += int(fh.size * 1.16)

    # subhead
    fs = font(REG, 46)
    y += 14
    d.text((92, y), sub, font=fs, fill=MUTED)
    y += int(fs.size * 1.9)

    # the real capture, rounded, centred, filling the rest
    shot = Image.open(RAW / f"{name}.png").convert("RGB")
    avail_h = H - y - 96
    scale = min((W - 184) / shot.width, avail_h / shot.height)
