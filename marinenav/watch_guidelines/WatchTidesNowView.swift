...
final class WatchTidesViewModel: ObservableObject {
    ...
    init(stationID: String = WatchStationStore.shared.selectedTideStationID,
         unit: TideUnit = WatchStationStore.shared.unit) {
...
struct WatchTidesNowView: View {
    ...
    private func resetScrub(_ snap: WatchTidesViewModel.Snapshot) {
        ...
        crownMinute = Date().timeIntervalSince(snap.windowStart) / 60
    }
