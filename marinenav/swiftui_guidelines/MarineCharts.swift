...
struct TideCurve: View {
            ...
            GeometryReader { geo in
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    ForEach(markers) { m in
                        ...
                        .monospacedDigit()
                        .fixedSize()
                        .position(x: mx, y: my + (m.positive ? -26 : 26))
                    }
    ...
    private func markerY(_ m: ChartMarker, in size: CGSize) -> CGFloat {
        let i = Int((m.x * Double(max(samples.count - 1, 1))).rounded())
        guard samples.indices.contains(i) else { return size.height / 2 }
        return y(samples[i], in: size)
    }
}
            ...
            GeometryReader { geo in
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    ForEach(markers) { m in
                        ...
                        let i = Int((m.x * Double(max(samples.count - 1, 1))).rounded())
                        let my = samples.indices.contains(i) ? y(samples[i], in: size) : zero
                        if m.hollow {
                            Circle()
                            ...
                            .fixedSize()
                            .position(x: mx, y: my + (m.positive ? -24 : 24))
                        }
                    }
