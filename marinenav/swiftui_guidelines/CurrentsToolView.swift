...
final class CurrentsViewModel: ObservableObject {
    ...
    var station: CurrentStation { record.station() }
    ...
    var series: [Double] {
        (0..<96).map {
            Currents.velocity(station,
                              at: windowStart.addingTimeInterval(Double($0) * 15 * 60))
        }
    }

    var events: [CurrentEvent] {
        Currents.events(station, start: windowStart, hours: 24)
    }

    var now: Double { Currents.velocity(station, at: Date()) }
        ...
        ToolScreen(tool: .currents) {
            ...
            HeroReadout(value: model.format(abs(model.knots(model.now))),
                ...
                .overlay(alignment: .topTrailing) {
                    Text(String(format: "%.0f", model.setNow))
                        .accessibilityIdentifier("result.nowSet")
                        .hidden()
                }
        ...
        MarineCard {
            HStack(spacing: 10) {
                ...
                Picker("Station", selection: $model.stationKey) {
                    ForEach(StationCatalog.currentStations) { s in
