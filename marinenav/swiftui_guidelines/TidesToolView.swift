...
final class TidesViewModel: ObservableObject {
    ...
    var station: Station { record.station(unit: unit) }
    ...
    /// Heights every 15 minutes across the day, for the curve.
    var curve: [Double] {
        Harmonics.heights(station, from: windowStart, count: 96, stepSeconds: 15 * 60)
    }

    var extremes: [TideEvent] {
        Harmonics.extremes(station, start: windowStart, hours: 24)
    }

    var nowHeight: Double { Harmonics.height(station, at: Date()) }

    /// Rate of change right now — rising or falling, and how fast.
    var nowSlope: Double { Harmonics.slope(station, at: Date()) }
    ...
    var nowLabel: String? {
        ...
        return "\(time(Date())) · \(format(nowHeight)) \(unitLabel)"
    }
    ...
    var countdown: String? {
        ...
        let s = Int(next.date.timeIntervalSince(Date()))
        ...
        ToolScreen(tool: .tides) {
            ...
            HeroReadout(value: model.format(model.nowHeight),
                ...
                .overlay(alignment: .topTrailing) {
                    // The slope keeps its own identifier for the UI test.
                    Text(model.format(abs(model.nowSlope)))
                        .accessibilityIdentifier("result.nowSlope")
                        .hidden()
                }
            ...
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(model.record.name)
                        ...
                        .foregroundStyle(theme.palette.ink)
                    Text("\(model.record.region) · \(model.record.id)")
                        .font(MarineType.caption)
                        .foregroundStyle(theme.palette.inkDim)
                }
                Spacer(minLength: 8)
